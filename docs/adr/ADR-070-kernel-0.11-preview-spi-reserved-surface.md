...
5. **Both new records are class-level `@JsonInclude(NON_NULL)`, not `NON_DEFAULT`.** This is load-bearing for `ScheduleMetadata` specifically: `TriggerKind.CRON` is ordinal 0, and under `NON_DEFAULT` an explicit `CRON` would be dropped on serialization and read back as `null` — the exact trap ADR-059 obligation 3 spelled out for a dropped `GLOBAL`, except that here there is no fallback accessor to catch it, so the schedule would silently cease to exist. `BlobMetadata` takes the same posture for consistency rather than for a live hazard.
