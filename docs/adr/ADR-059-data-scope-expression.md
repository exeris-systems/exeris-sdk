...
3. **Wire-format guard** — `AstJsonRoundTripTest` covers every tier explicitly, including `GLOBAL`. `DomainMetadata` is class-level `@JsonInclude(NON_NULL)` — the deliberate `ViewMetadata` posture — so the ordinal-0 / boxed-zero trap that cost the `FieldMetadata` bounds a fix in 0.9.0 is **not armed** on this record, and a non-null tier is not at risk today. Every tier is pinned anyway because the cost of that posture changing is asymmetric: under `NON_DEFAULT` it is `GLOBAL` (ordinal 0) that drops, and a dropped explicit `GLOBAL` is not a lost hint — it falls back through `effectiveDataScope()` and reads the entity back as `TENANT`. The tier that looks least interesting is the one whose loss is a silent tenancy flip. A second case pins that an **absent** tier stays absent on the wire rather than being materialised by the reader.

   *(Amended 2026-08-18, with ADR-072: **the asymmetry argument in the preceding sentences is wrong.** `NON_DEFAULT` drops a boxed numeric zero, which is the caveat `CLAUDE.md` states and the defect that cost the `FieldMetadata` bounds a fix in 0.9.0 — but Jackson does not treat an ordinal-0 **enum** constant as empty, and `GLOBAL` survives `NON_DEFAULT` untouched. There is no silent tenancy flip behind a posture change. The claim was never exercised, precisely because `DomainMetadata` is `NON_NULL`, so nothing ever tested the premise; it was carried forward as reasoning for two more inclusion decisions before anyone measured it. `AstJsonRoundTripTest` now pins the actual semantics. Nothing in this obligation changes in substance: `NON_NULL` remains right for this record, and pinning every tier explicitly remains worth doing — the round-trip cases are cheap and a tier dropping for any *other* reason would still be caught. Only the stated justification was false.)*
