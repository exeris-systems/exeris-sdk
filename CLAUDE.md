...
`tests/public-surface.txt` is the enforced half: 126 recorded names, snapshot-gated the way
...
| `exeris-sdk-annotation-catalog` | **no** (jar) | Build-time generator for `annotation-catalog.json` / `ast-schema.json`; nobody resolves it. Held back **twice**, because the two settings govern different plugins: `<maven.deploy.skip>true</maven.deploy.skip>` in its own pom stops `maven-deploy-plugin`, and an `<excludeArtifacts>` entry in the root pom's `release` profile stops `central-publishing-maven-plugin`, which replaces that plugin and does not read the property. `release.yml` asserts the two lists agree |
